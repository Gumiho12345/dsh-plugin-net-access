...
function parseArgs(raw) {
	...
	if (mode !== "read-only" && mode !== "workspace-write" && mode !== "net-access") fail(`unknown mode: ${String(mode)}`);
...
async function main() {
	...
	try {
		...
		let writeSid;
		let privateTempSid;
		if (parsed.mode === "workspace-write" || parsed.mode === "net-access") {
		...
		sandbox = new AclSandbox({
			writableDirs: parsed.mode === "workspace-write" || parsed.mode === "net-access" ? [parsed.workspace] : [],
