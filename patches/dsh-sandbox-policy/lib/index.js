...
function renderPolicyContext(policy) {
	switch (policy.mode) {
		...
		case "net-access": return `Current DSH file policy: net-access. File modifications are confined to the session workspace (${JSON.stringify(policy.workspaceRoot)}) and permitted temporary areas, and group-dependent Windows APIs (WMI/CIM, reads) are restored. Note: Schannel/outbound SSPI (HTTPS via curl or Invoke-WebRequest) cannot work under any restricted-token mode on Windows — use a non-SSPI TLS client (python/openssl) or escalate to danger-full-access.`;
